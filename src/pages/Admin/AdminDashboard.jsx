...
import { LayoutDashboard, Users, ShoppingBag, DollarSign, Package, Settings, LogOut, Lock, CheckCircle, XCircle, TrendingUp } from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        if (activeTab === 'users') {
            ...
            setUsers([
                { id: 1, username: 'testuser', email: 'user@test.com', role: 'USER', isActive: true },
                { id: 2, username: 'testseller', email: 'shop@test.com', role: 'SELLER', isActive: true },
                { id: 3, username: 'badguy', email: 'spam@test.com', role: 'USER', isActive: false },
            ]);
        }
    }, [activeTab]);

    // === HÀM XỬ LÝ KHI BẤM NÚT STATS ===
    const handleViewSellerStats = async (sellerId, sellerName) => {
        setSelectedSeller(sellerName);
        setIsStatsModalOpen(true);
        setIsStatsLoading(true);
        try {
            // Gọi API lấy thống kê của riêng seller này
